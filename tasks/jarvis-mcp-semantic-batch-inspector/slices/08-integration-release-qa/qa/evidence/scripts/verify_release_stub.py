...
ROOT = Path(r"C:/Projects/jarvis/sub-agents/jarvis-agent-01")
...
def main() -> None:
    result = subprocess.run(
        [sys.executable, "-c", "print(\"[stub] pytest skipped\")"],
        cwd=ROOT,
        text=True,
        check=False,
    )
    ...
    for needle in dangerous:
        if needle in source:
            print(f"WARN (pre-existing, barehands_replay.py): {needle}")
