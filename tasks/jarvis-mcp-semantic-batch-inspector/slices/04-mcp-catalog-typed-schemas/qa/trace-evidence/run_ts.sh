...
$PY $S/fake_api_ts.py $APIPORT $S/script_ts.json $R/bodies.jsonl > $R/api.log 2>&1 &
API=$!
...
sleep 1
cd $R
ENABLE_TOOL_SEARCH=true ANTHROPIC_BASE_URL=http://127.0.0.1:$APIPORT ANTHROPIC_API_KEY=sk-ant-dummy-not-real CLAUDE_CONFIG_DIR=$(cygpath -w $R/cfg) \
