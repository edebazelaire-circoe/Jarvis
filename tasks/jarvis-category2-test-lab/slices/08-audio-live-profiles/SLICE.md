# Slice 08 — Controlled audio and live-provider profiles

## Goal
Add progressively more realistic `audio` and `live` profiles using controlled audio chain and real provider only when declared.
...
## Scope
### In Scope
Reuse existing duplex/AEC/audio diagnostics and realtime adapter boundaries. The `audio` profile should exercise controlled local audio processing without requiring a provider where possible. `live` may call the real provider behind explicit capability/budget gates. Record provider/model/config identity and cost metadata.
...
## Dependencies
`05-supervisor-worker`, `07-metrics-scoring-sweeps`
