# Slice 05 — Run supervisor and isolated worker lifecycle

## Goal
Run every TestRun in a supervised isolated worker process with cancellation, timeout, cleanup, crash capture, and resource reservation.
...
## Scope
### In Scope
Implement supervisor/worker IPC around the Test Lab contracts. Snapshot config and apply run-local overrides only. Reserve mutually exclusive resources (provider/device/human session as applicable). Persist terminal state even on worker crash. Never run long/physical diagnostics inside the Control Center process.
...
## Dependencies
`01-testlab-domain-contracts`, `02-run-persistence-artifacts`, `04-catalog-scenarios`
