...
session token file is re-read (Core restarted) and the same batch is resent once
at once (Slice 10; nothing of a refused batch was applied); HTTP 400 → the batch is dropped
...
whose work vanished are not touched: only a Core start marks stars (see *Restart
reconciliation* below); a `store_changed` reconciliation never marks nor wipes.
...
`SceneUnavailableError`; a start failure stops them in the same order. The
projector's `stop()` also cancels and awaits the restart grace timer
(`jarvis-scene-restart-grace`) after the drain and the space watcher, so no task
outlives `_stop_scene()`, whether the loop was started or not.

#### Restart reconciliation

Handoff Slice 10 (decisions 11, 12, 13, 17). The scene is durable, Core work
state is not: after a Core restart the work store is empty under a new
`store_id`, so a persisted star that still says `running` says nothing true. The
reconciler lives **in the projector** (`SceneProjector.reconcile_restart`,
`_expire_grace`, `_interrupt_unobserved` in `jarvis/core/scene_projector.py`):
the grace expiry must run inside the projector's serialised loop (an observation
must not slip between the read and the write) and reuses its deferral, catch-up
priority, signal lifecycle and journal; a separate module would have had to
...
fresh grace. A kill between the signal and the star write leaves a live
`core_restarted_unobserved` signal on an `unknown` star: the next expiry's
`attach_signal` is a `duplicate` (no `signal_raised`), then the star is
interrupted. A kill after both writes leaves an `interrupted` star, terminal,
never re-marked. No path raises a second signal for one work item.

Grace value. `RESTART_GRACE_S` = **60 s** (production default),
`JarvisCoreApplication(scene_restart_grace_s=)`, env
`JARVIS_SCENE_RESTART_GRACE_S` (`]0, 3600]`; an invalid value keeps the default
and is journaled `core.scene.restart_grace_invalid`, warning). Justification:
the Control Center's `WorkIngressForwarder` resends its tracker's full state at
most 30 s after its last send while idle, and its retry backoff caps at 30 s
when Core stayed down long. `CoreWorkTransport.post` now re-reads the token and
resends the same batch **once, immediately** on a 401 (a 401 means nothing was
applied), instead of waiting for another backoff period: the first send to a
restarted Core succeeds at worst ≈ 31 s after Core starts, and the full resend
follows in the next 0.5 s flush. 60 s covers two resend periods, so one slow or
lost resend does not interrupt live work. Below 30 s, live sub-agents of an idle
Control Center would be interrupted wrongly; the env override exists for
validation runs.

Restart paths.

| Path | What happens | Scene result |
| --- | --- | --- |
...
| Browser reload | Slice 05: snapshot on load, patches after; a new Core epoch forces a resync | the page shows `unknown` stars with their cue, then the interruption signal as patches |
| Completed star and artifacts | never tracked, never written | unchanged across Core and Control Center restarts (decision 12) |

Journal. `core.scene.restart_marked` (info, once at start: `marked`,
`already_unknown`, `tracked`, `terminal_untouched`, `job_outcomes`, `grace_s`,
`sample` = up to 16 star ids), `core.scene.restart_grace_expired` (info, once at
expiry, only when something was tracked: `tracked`, `reobserved`, `interrupted`,
`deferred`, `left`, `failed`, `grace_s`), `core.scene.restart_star` (level
`debug`, one per star: `action` = `unknown` with `previous`, `reobserved`,
`interrupted`, `left`, `failed`), plus `core.scene.signal_raised` with
`error_class = core_restarted_unobserved` for each new signal.
`SceneProjector.stats` gains `restart_marked`, `restart_reobserved`,
`restart_interrupted`; `restart_tracked_count` exposes the tracked set.
...
- `exec_state` is a secondary cue only: breathing ring (running), dashed ring
  ...
  check on terminated capsules and windows. Slice 10: an execution node
  (`agent`/`job`) in `unknown` (`restartUnknown` in the view model) reads
  « état inconnu depuis le redémarrage » in its label, gets class
  `sc-restart-unknown` (pale dotted ring, never animated, dimmed mark) and a
  « ? » badge; neither running nor an alert. `unknown` on other kinds keeps no
  label and no badge. The error class `core_restarted_unobserved` displays as
  « non revu après le redémarrage de Core » (medium urgency).
...
- `holdNode` counts the hands on each object: the first one to let go does not release an object the keyboard still holds, and the keyboard and Bare Hands exclude each other on one object (as the mouse already did).
- **Second QA round.** The mouse hold starts at the drag threshold (4 px), and the object keeps turning between the press and the threshold; the hold takes it back to where it was drawn at the press (`hold.shift`), so nothing drifts under the pointer (2.9 px at speed 1, up to 20 px at speed 4 before). The object is deliberately **not** frozen at the press: a plain click would then make it jump forward along its orbit when released, by the whole duration of the press; instead, at the threshold it is set back once, under the hand. The rebase runs at the first event that sees a new window (pointer move, Bare Hands frame, key, render), re-bounds the held objects into the new visible area, and the walls follow controls that appear during a gesture (`hold.setArea`). Animations are set to the wall-clock time **of the frame on screen** (`orbitFrameWall`: `Date.now()` taken back to `document.timeline`), which is what they advance on — a tab coming back with a stale document clock was otherwise put ahead by the whole lag (5–12 px on untouched objects) — and a drift guard compares the two clocks at the same frame every 2 s (`orbitClockGap`, no false positive on slow frames). A speed or field change resyncs every animation that shares the clock, the thread layer included.
- Above amplitude 1, a still object right at the edge of its ellipse and pushed more than about a pixel inward **starts turning**: no still place draws it under the pointer any more (an object turns when it is inside its ellipse), and the drop clause (≤ 1 px) wins over keeping its stored place; its drawing stays within a pixel.
...
| `control_center_barehands_hand_art.js` | `JarvisBarehandsHandArt` | Slice 04 of the UI refinement. The **single schematic-hand drawing vocabulary** for the whole feature (decision 20): seven poses held as data — digit polylines, joints, contact and target rings — and one serialiser pair (`handSvg` / `handMarkup`) built from one pure `handShapes`. It **knows nothing of gestures**: it names no `GESTURE`, reads no contract, and never says what a pose triggers. That binding belongs to whoever displays. Inserted before the calibration, which consumes it, and before the HUD, which takes its button icon from it. |
...
| `control_center_barehands_calibration.js` | `JarvisBarehandsCalibration` | Slice 08. The calibration flow **and the overlay shell** (§11). The shell knows nothing of what it displays — it shows steps — which is what let the tutorial reuse it unchanged (decision 26); that tutorial was **retired in Slice 07B of the UI refinement** and there is now exactly one guided flow. Slice 06 of the UI refinement added the **step phase machine** (`INTRO → ARMED → RUNNING → RESULT → NEXT`): reading time is not measurement time, the stage deadline exists only under `RUNNING`, and an idle user waits in `ARMED` indefinitely without anything counting down. The flow now draws, so it requires a `document` and the hand-drawing vocabulary above, both refused absent at construction. |
...
| `control_center_barehands_hud.js` | `JarvisBarehandsHud` | The upper-left HUD lifecycle control: a hand-icon button and a three-state visual chooser (OFF / SLEEP / ACTIVE). Holds **no** lifecycle state — it subscribes to `JarvisBarehands.openLifecycleSeam` and drives transitions through the same entry points the settings panel and the voice channel use. Slice 03 adds the **fixed left tool palette** to the same module — same family, same screen corner, same tone-ladder discipline: it is populated from `describeTools()`, writes through `JarvisBarehands.tool()`, and subscribes to both `openToolSeam` and `openLifecycleSeam`. The two surfaces install in **separate** `try` blocks, so a missing palette host never costs the lifecycle control. Slice 04 adds the two **quick cards** — Help / Gestures and Diagnostics — as a third surface in a third `try`, sharing one host and one stylesheet with the other two. They subscribe to no seam: the help is still, and the diagnostics card re-reads `record.state()` once a second while it is open. |
...
```
CONTRACTS → TARGET → CALIBRATION → RECORDER → BAREHANDS → COMMANDS
```
