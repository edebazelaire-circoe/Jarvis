...
instead of printing `0.0`. A figure restated a second time in prose is compared
again at that second place, not only at its first. `--key <clé>` prints one
figure with its source file and JSON path.

**What is not.** Four kinds of statement are transcribed by hand and **not**
asserted: the loose approximations `0.8–0.9 for up to ~1.5 s` and
`strict zero-FA thresholds ≈ 0.9 → FRR > 90 %` (observation 3), `≈ 2 % FRR`
(observation 2) and the `~2–3×` strict-over-settled timing ratio in the host
note of the 2026-09-11 run (an eyeballed range over several engines, not one
quotient); the counts restated in words ("one judged 100 ms window in
