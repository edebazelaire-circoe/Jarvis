...
owner profile are all usable, Voice (`continuous_brain`) scores the AEC-cleaned
capture and logs `voice.owner.*` events, without changing a single byte of what
reaches the provider. The screen reports the real verifier state (`verifier`, plus
...
```bash
JARVIS_LIVE_OPENAI=2 OPENAI_API_KEY=... python -m pytest -q tests/integration/test_live_openai.py
```
