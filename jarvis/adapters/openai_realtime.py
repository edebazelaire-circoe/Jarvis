...
class OpenAIRealtimeSession:
    ...
    @classmethod
    async def connect(cls, *, api_key: str, model: str, voice: str, context: dict[str, object], tools: list[dict[str, object]] | None = None, session: aiohttp.ClientSession | None = None) -> "OpenAIRealtimeSession":
        ...
        try:
            ...
            await ws.send_json({"type": "session.update", "session": {"instructions": instructions, "voice": voice, "modalities": ["audio", "text"], "tools": tools or [], "tool_choice": "auto", "turn_detection": {"type": "server_vad", "create_response": True, "interrupt_response": True}}})
            return instance
        except Exception:
            if owns:
                await http.close()
    ...
    async def send_audio(self, pcm: bytes) -> None:
        await self.ws.send_json({"type": "input_audio_buffer.append", "audio": base64.b64encode(pcm).decode("ascii")})
    ...
    async def events(self) -> AsyncIterator[ProtocolEnvelope]:
        async for message in self.ws:
            ...
            if kind in {"response.audio.delta", "response.output_audio.delta"}:
            ...
            elif kind in {"conversation.item.input_audio_transcription.completed", "input_audio_buffer.transcription.completed"}:
                yield ProtocolEnvelope(message_type="realtime.transcript", payload={"text": data.get("transcript", "")})
            elif kind in {"response.audio_transcript.done", "response.output_audio_transcript.done"}:
                yield ProtocolEnvelope(message_type="realtime.assistant_transcript", payload={"text": data.get("transcript", "")})
                ...
                if item.get("type") == "function_call" or kind == "response.function_call_arguments.done":
                    ...
                    try:
                        ...
                        arguments = {}
                    yield ProtocolEnvelope(message_type="realtime.tool_call", payload={"call_id": item.get("call_id") or data.get("call_id"), "name": item.get("name") or data.get("name"), "arguments": arguments})
            elif kind == "error":
