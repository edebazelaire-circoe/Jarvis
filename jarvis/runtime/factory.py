...
from jarvis.adapters.null_board import NullBoardClient
...
def create_runtime(config: AppConfig, *, speech_enabled: bool) -> RuntimeBundle:
    ...
    logger = build_logger(
        ...
        log_content=config.runtime.log_content,
    )
    memory = MarkdownMemoryBackend(config.runtime.memory_dir)
    board = BarehandsBoardClient(config.board.url, "v1-local") if config.board.enabled else NullBoardClient()
    ...
    if speech_enabled:
        tts = OpenAITTSBackend(
            ...
            instructions=config.openai.tts_instructions,
            base_url=config.openai.base_url,
            timeout_s=config.openai.timeout_s,
        )
    else:
    ...
    broker = ActionBroker(
        build_action_executors(memory, board),
        confirmation_timeout_s=config.runtime.confirmation_timeout_s,
    )
    orchestrator = JarvisOrchestrator(
        agent=agent,
        tts=tts,
